...
const OrderDetails = () => {
  const { id } = useParams();
  const { orderDetails, loading, fetchOrderDetails } = useContext(OrderContext);
  ...
  useEffect(() => {
    (async () => await fetchOrderDetails(id))();
  }, [id]);
  ...
  return user && (
    <div className="w-full max-w-7xl mx-auto p-4 sm:p-6 ">
      ...
      {!orderDetails ? (
        ...
        <div className="p-4 sm:p-6 rounded-lg border bg-white shadow-lg ">
                ...
                {orderDetails.orderItems.map((item) => (
                  <tr key={item.productId} className="border-b">
                    <td className="py-2 px-4 flex items-center">
                      <img
                        src={item.image.replace("/upload", "/upload/w_150")}
                        alt={item.name}
